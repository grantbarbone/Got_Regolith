...
from radariq_msg.msg import RadarIQObject, RadarIQObjects
...
class ObjectPublisher(Node):
    def __init__(self):
        ...
        framerate = self.declare_parameter('framerate', 10).value  # Default to 10 if not set
        distancefilter_min = self.declare_parameter('distancefilter_min', 0.0).value
        distancefilter_max = self.declare_parameter('distancefilter_max', 100.0).value
        anglefilter_min = self.declare_parameter('anglefilter_min', -3.14).value
        anglefilter_max = self.declare_parameter('anglefilter_max', 3.14).value
        pointdensity = self.declare_parameter('pointdensity', 10).value
        certainty = self.declare_parameter('certainty', 50).value
        marker_topic = self.declare_parameter('marker_topic', 'marker_topic').value
        object_data_topic = self.declare_parameter('object_data_topic', 'object_data_topic').value
        self.frame_id = self.declare_parameter('frame_id', 'base_link').value

        if not (0 <= framerate <= 20):
            self.get_logger().error(f"Frame rate must be between 0 and 20 fps, but got {framerate}.")
            return
    ...
    def timer_callback(self):
        ...
        try:
            row = riq.get_data()  # Fetch data from RadarIQ
            if row is not None:
                markers = self.build_marker_array(row)
                objs = self.build_object_array(row)
            ...
            self.get_logger().error(f"Error in data processing: {error}")
