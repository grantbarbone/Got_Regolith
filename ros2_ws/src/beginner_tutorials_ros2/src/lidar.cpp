...
class Lidar : public rclcpp::Node {
 public:
  ...
  Lidar() : Node("Lidar_PubSub") {
    publisher_ = this->create_publisher<std_msgs::msg::UInt16MultiArray>("topic_lidar_to_main", 10);
    lidar_timer_ = this->create_wall_timer(
        500ms, std::bind(&Lidar::timer_callback, this));
        
    subscription_ = this->create_subscription<std_msgs::msg::Bool>(
      "topic_main_to_lidar", 10, std::bind(&Lidar::topic_callback, this, std::placeholders::_1));
  }
 ...
 private:
  ...
  void timer_callback() {
    ...
    RCLCPP_INFO(this->get_logger(), "Publishing message size: [%s]", std::to_string(message.data.size()).c_str());
